...
void Query::setName(const std::string &name) { queryName = name; }
void Query::setValue(const std::string &value) { queryValue = value; }

void Query::setComparisonOperator(ComparisonOperator op) { queryOp = op; }
...
bool Query::hasError() { return error; }

std::string Query::getErrorMessage() { return errorMessage; }

bool Query::isMatch(const google::protobuf::Descriptor *descriptor,
  ...
  const google::protobuf::FieldDescriptor *fd =
      descriptor->FindFieldByName(queryName);
  ...
  switch (type) {
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_STRING: {
      ...
      queryValue.erase(remove(queryValue.begin(), queryValue.end(), '\''),
                       queryValue.end());
    ...
    default: {
      setError(
          "This field cannot be used in search queries because of its "
          "datatype: " +
          queryName + "(" + fd->type_name() + ")");
      break;
    }
  }
  return output;
